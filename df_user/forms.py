    ...
    def clean(self):
        ...
        if user is None:
            if UserModel.objects.filter(email=username_or_email).exists():
                username = UserModel.objects.get(email=username_or_email).username
...
class ChangeNicknameForm(forms.Form):
    ...
    def clean_nickname_new(self):
        username_new = self.cleaned_data.get('username_new', '').strip()
        if username_new == '':
            raise forms.ValidationError("新的用户名不能为空")
        return username_new
